...
async function main() {
  ...
  try {
    ...
    console.log('PASS: headless Chrome verified project navigation, keyboard focus, selection, binding copy, task-detail copy/fallback, and synthetic issued-token copy/fallback.');
  } finally {
    socket?.close();
