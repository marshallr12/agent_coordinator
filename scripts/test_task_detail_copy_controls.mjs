...
async function main() {
  ...
  try {
    ...
    await evaluate("(() => { const size = document.querySelector('#tasks-page-size'); size.value = '50'; size.dispatchEvent(new Event('change', { bubbles: true })); })()");
    await waitPage("document.querySelectorAll('.task-row').length === 30 && document.querySelector('#tasks-page-status').textContent === 'Page 1 of 1'", 'latest 50-item selection');
    await new Promise((resolveDelay) => setTimeout(resolveDelay, 600));
    assert(await evaluate("document.querySelectorAll('.task-row').length === 30 && document.querySelector('#tasks-page-size').value === '50' && document.querySelector('#tasks-page-status').textContent === 'Page 1 of 1'"), 'A stale delayed response replaced the latest 50-item selection.');
