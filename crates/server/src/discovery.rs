...
pub const INSPECTION_INSTRUCTIONS: &str = "Use coordinator_preconditions_get with a task or activity ID to inspect current claim/review/integration blockers before attempting guarded work. Task, activity, and individual job reads include state_token values. coordinator_state_wait can wait up to 30 seconds for one task work_status, workflow activity, or job state change; it polls every 500 ms without holding a database transaction or renewing ownership. These observations can become stale immediately. For code integration, Git merge-conflict status requires a local target preflight; the service does not inspect workstation repositories.";
