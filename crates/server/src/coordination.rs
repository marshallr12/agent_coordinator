...
async fn orientation(State(s): State<AppState>, auth: Auth, Path(p): Path<String>) -> Reply {
    ...
    Ok(response(
        ...
        "candidates":candidates.iter().map(|t|t.value(now)).collect::<Vec<_>>(),"active_attempts":active.iter().map(Attempt::value).collect::<Vec<_>>(),"recovery_candidates":recovery.iter().map(|t|t.value(now)).collect::<Vec<_>>(),"implemented_stage":"operator_workflows", "operator_tools": {
