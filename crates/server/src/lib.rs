...
async fn authentication_help() -> Json<Value> {
    response(
        ...
        "mcp_steps":["Prefer a configured authenticated MCP connection for coordination; no native CLI is required. Supply protected bearer/session/proof headers from the host configuration.","Inspect coordinator_session_get or register a newly provisioned session with coordinator_session_register; read and acknowledge complete orientation before claiming.","Use coordinator_claim, coordinator_attempt_renew, coordinator_checkpoint and coordinator_attempt_release. Persist mutation keys and arguments before sending; reuse them on uncertainty.","Local worktree/job/Git operations require native capabilities. Adopt the same quiescent protected session with session adopt-mcp or checkpoint/release before claiming through another session.","Read data.agent_startup.guide at /api/v1/info for full startup, missing-client and transition instructions."],
