...
async fn authentication_help() -> Json<Value> {
    response(
        ...
        "mcp_steps":["Prefer a configured authenticated MCP connection for coordination; no native CLI is required. Supply protected bearer/session/proof headers from the host configuration.","Inspect coordinator_session_get or register a newly provisioned session with coordinator_session_register; read and acknowledge complete orientation before claiming.","Use coordinator_claim, coordinator_attempt_renew, coordinator_checkpoint and coordinator_attempt_release. Persist mutation keys and arguments before sending; reuse them on uncertainty.","When policy permits, automatically use an eligible stable non-contributing subagent for review. Treat its result as intermediate, refresh live workflow state, and continue without another user prompt unless the user explicitly requested review-only or stop.","Local worktree/job/Git operations require native capabilities. Adopt the same quiescent protected session with session adopt-mcp or checkpoint/release before claiming through another session.","Read data.agent_startup.guide at /api/v1/info for full startup, missing-client and transition instructions."],
        ...
        "startup_guide":"/api/v1/info#data.agent_startup.guide",
        "bootstrap_snippet":discovery::agent_startup()["local_bootstrap"],
