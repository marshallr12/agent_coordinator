...
**Status: coordination, Linux operations, MCP, and this documentation book are
implemented; the operator-run physical Windows workstation exercise remains.**
...
The book separates operational guides and implemented contracts from preserved
release-design chapters. Treat [implementation status](docs/implementation-status.md)
as the source for working behavior; a proposed example in the plan or a historical
contract does not make an endpoint available.
...
For remote use, follow [the Linux/systemd/HTTPS examples](deploy/).
