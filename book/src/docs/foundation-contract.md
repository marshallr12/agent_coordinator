...
Status: historical integration contract for the first implementation slice. See
[implementation status](implementation-status.md) for the complete current service.

This file coordinated parallel implementation, not a change to product scope.
...
- `state::AppState`: Clone, public `pool: sqlx::SqlitePool`, `config: Config`,
  `clock: Arc<dyn Clock>`; `now() -> i64` milliseconds;
  `open(Config) -> anyhow::Result<Self>`. Clock: Send + Sync +
  `fn now_ms(&self) -> i64`.
...
- `error::AppError`: `new(status: StatusCode, code: &str, message: &str)`,
  ...
  `From<sqlx::Error>`, `serde_json::Error`; `IntoResponse` JSON envelope. No
  secret Debug.
- `response(data: Value) -> axum::Json<Value>` produces
  ...
  fields `tx: Transaction<'static, Sqlite>`, `actor: Actor`, `now: i64`, and
  `replay: Option<Value>`; `finish(...) -> Result<Value, AppError>` returns data
  after commit.
