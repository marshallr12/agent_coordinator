...
Status: historical implementation design supporting [the plan](../PLAN.md).
The coordination service now implements the core ownership and recovery rules,
but current routes, fields, and limits are recorded in
[implementation status](implementation-status.md) and the operational guides.
This chapter preserves the correctness rationale and tunable engineering defaults.
