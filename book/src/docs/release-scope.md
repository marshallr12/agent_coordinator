...
Status: historical implementation-readiness record. Confirmed choices are in
[PLAN.md](../PLAN.md). Engineering defaults remain distinguishable from
operator-selected requirements; completed validation and remaining acceptance
work are recorded in [implementation status](implementation-status.md).
